...
class PlayPiece {
  ...
  futureCollision(x, y, rotation) {
    ...
    for (let i = 0; i < this.pieces.length; i++) {
      if (points) {
        xx = this.pos.x + points[1][0] * gridSpace;
      ...
      for (let j = 0; j < gridPieces.length; j++) {
        if (xx === gridPieces[j].pos.x) {
          if (
            ...
            yy < gridPieces[j].pos.y + gridPieces
          ) {
            return true;
          }
          if (
...
//Analyse the grid and clear lines if necessary
